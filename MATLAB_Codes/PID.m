...
tolerance_band = 0.02; % ±10% for settling time

%% Simulate
out_linear = sim('PID_controller.slx');
out_nonlinear = sim('cruise_control_copy.slx');

%% Extract
v = out.v_nonlinear;
disp(v)
v_linear = out_linear.v_nonlinear;
...
title(sprintf('Linear Plant: v_{desired} = %.2f m/s\nOS = %.2f%%, Settling Time = %.2f s', desired_speed, overshoot_lin, t_settle_lin), 'FontSize', 14);
legend('Linear Velocity', 'Desired Speed', 'Location', 'best', 'FontSize', 11);

xlim([0 sim_duration]);
ylim([0 1.2 * max([max(v_linear), max(v_nonlinear)])]); % Dynamic Y-limits
...
xlabel('Time (s)', 'FontSize', 12);
ylabel('Velocity (m/s)', 'FontSize', 12);
title(sprintf('Nonlinear Plant: v_{desired} = %.2f m/s\nOS = %.2f%%, Settling Time = %.2f s', desired_speed, overshoot_nl, t_settle_nl), 'FontSize', 14);
legend('Nonlinear Velocity', 'Desired Speed', 'Location', 'best', 'FontSize', 11);

xlim([0 sim_duration]);
ylim([0 1.2 * max([max(v_linear), max(v_nonlinear)])]); % Same dynamic Y-limits
